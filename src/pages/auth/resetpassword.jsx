...
export default function ResetPassword() {
  const [haveUpper, setHaveUpper] = useState(false);
  const [haveLower, setHaveLower] = useState(false);
  const [haveNumber, setHaveNumber] = useState(false);

  const numbers = /\b([0-9]|10)\b /;
  const upperCase = /^[A-Z][a-z][A-Za-z]*$/;
  ...
  const formik = useFormik({
    ...
    validationSchema: object({
      newPassword: string().required("New password is requierd field").min(12),
